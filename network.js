  ...
  const I18N = {
    en: {
      ...
      <p>The project is currently supported by a research grant from <a href="https://www.gerda-henkel-stiftung.de/" target="_blank">Gerda Henkel Stiftung (Gerda Henkel Foundation).</a></p>
      ...
      <p><strong>Methodology:</strong> <u>Art historical research</u>: based on systematic analysis of bibliographic and archival sources (such as correspondence) from 19th-century France and Spain. Network nodes and edges were constructed through careful cross-referencing of academic publications (monographs, peer-reviewed articles, exhibition catalogues), institutional websites and online collection databases of museums and cultural foundations, and verified digital resources.<br>
      <u>Web development</u>: built using HTML and JavaScript, with structured data in JSON, and version control and deployment through GitHub, developed with the assistance of ChatGPT (OpenAI), DeepSeek (High-Flyer), and Claude (Anthropic). Initial website setup in collaboration with Anita Verő <a href="https://www.consonant-ai.com" target="_blank">(ConsonantAI)</a>.</p>
      ...
      <ul style="padding-left:1.2rem; line-height:1.7;">
        <li><strong>Leonardo (MIT Press)</strong> – <a href="https://direct.mit.edu/leon/article-abstract/doi/10.1162/LEON.a.2761/137543/The-Goya-Network-Mapping-Goya-s-Impact-in-19th?redirectedFrom=fulltext" target="_blank">Read more</a> Paula Fayos Pérez & Anita Verő, “The Goya Network: Mapping Goya's Impact in 19th-Century France and Spain”.</li>
      </ul>

      <h3 style="margin-top:1.5rem; margin-bottom:0.8rem;">Conferences and presentations</h3>
      <ul style="padding-left:1.2rem; line-height:1.7;">
        <li><strong>Instituto Cervantes London (2026)</strong> – <a href="https://www.eventbrite.co.uk/e/goya-network-tickets-1991696763635?aff=oddtdtcreator" target="_blank">(see event)</a> 'Goya Network: Francisco de Goya's influence on 19th-century French art and literature'.</li>  
        <li><strong>Congreso Humanidades Digitales Hispánicas 2025 (Universidad de Valencia)</strong> – <a href="https://hdh2025.uv.es/static/files/Programa_Congreso_1_8.pdf" target="_blank">(view program)</a> 'La Red Goya: Humanidades Digitales para mapear la recepción de Goya en el siglo XIX'.</li>
        <li><strong>Asociación Humanidades Digitales Hispánicas (2025)</strong> – <a href="https://humanidadesdigitaleshispanicas.es/cafe-con-paula-fayos-perez-16-de-diciembre-de-2025/" target="_blank"></a> 'Un café con Paula Fayos Pérez. La Red Goya: Humanidades Digitales para mapear la recepción de Goya en el siglo XIX'.</li>
      </ul>

      <h3 style="margin-top:1.5rem; margin-bottom:0.8rem;">Online presence/mentions</h3>
      <ul style="padding-left:1.2rem; line-height:1.7;">
        <li><strong>Artes UK</strong> – <a href="https://artes-uk.org/links-2/" target="_blank">Iberian & Latin American Visual Culture Group</a>.</li>
      ...
      <ul style="padding-left:1.2rem; line-height:1.7;">
        <li><strong>MasDeArte</strong> – <a href="https://masdearte.com/especiales/paula-fayos-perez/" target="_blank">Paula Fayos Pérez in Especiales/Fichados</a>.</li>
      </ul>
      `,
    ...
    es: {
      ...
      <p><strong>Financiación:</strong> Este proyecto fue iniciado con el apoyo de una Beca Leonardo 2024 (<a href="https://www.redleonardo.es/beneficiario/paula-fayos-perez/" target="_blank">Beca Leonardo</a>) para Investigación Científica y Creación Cultural, financiada por la <a href="https://www.fbbva.es/" target="_blank">Fundación BBVA.</a> La Fundación BBVA no asume responsabilidad alguna por las opiniones, afirmaciones y contenidos de este proyecto, que son enteramente responsabilidad de sus autores.</p>

      <p>Actualmente el proyecto cuenta con el apoyo de una beca de investigación de la <a href="https://www.gerda-henkel-stiftung.de/" target="_blank">Gerda Henkel Stiftung (Fundación Gerda Henkel).</a></p>
      ...
      <p><strong>Metodología:</strong> <u>Investigación en Historia del arte</u>: basada en análisis sistemático de fuentes bibliográficas y de archivo (como correspondencia) de la Francia y España del siglo XIX. Los nodos y conexiones de la red fueron construidos mediante la cuidadosa comparación cruzada de publicaciones académicas (monografías, artículos revisados por pares, catálogos de exposiciones), sitios web institucionales y bases de datos de colecciones en línea de museos y fundaciones culturales, y recursos digitales verificados.<br>
      <u>Desarrollo web</u>: realizado con HTML y JavaScript, con datos estructurados en JSON, y control de versiones y despliegue mediante GitHub; desarrollado con el apoyo de ChatGPT (OpenAI), DeepSeek (High-Flyer) y Claude (Anthropic). Configuración inicial del sitio web en colaboración con Anita Verő <a href="https://www.consonant-ai.com" target="_blank">(ConsonantAI)</a>.</p>
...
function handleInitialHash(retryCount = 0) {
  ...
  if (type === 'edge') {
    ...
    let matchingEdge = edges.get().find(edge => {
      ...
      const pairNorm = [normalizeForComparison(fromNode.id), normalizeForComparison(toNode.id)].sort();
      return pairNorm[0] === leftNorm && pairNorm[1] === rightNorm;
    });
    ...
    if (matchingEdge) {
      setTimeout(() => {
        ...
        if (fromPos && toPos) {
          ...
          window.VIS_NETWORK.moveTo({position: {x: centerX, y: centerY}, scale: window.VIS_NETWORK.getScale(), animation: {duration: 500}});
        }
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    ...
    */
    ...
    // Check if we have a specific node/edge hash URL - for logging purposes only
    ...
    network.on("click", function (params) {
      ...
      if (params.nodes.length > 0) {
        ...
        const fieldsToShow = [
          ...
          { type: "section", label: "Social and Political Identity" },
          ...
          { type: "section", label: "Curiosities" },
          ...
          if (Array.isArray(value)) {
            ...
            if (typeof value === "string" && field.key === "profession") {
              translatedValue = value
                ...
                .map(v => {
                  v = v.trim();
                  // First translate the profession
                  let translated = translateValue(v);
                  // Then apply gender-specific adaptation
                  if (node.sex === "female") {
                    // For female: "pintor/a" becomes "pintora", "editor/a" becomes "editora"
                    translated = translated.replace(/(\w+)\/a\b/g, '$1a');
                  } else if (node.sex === "male") {
                    // For male: remove /a - "pintor/a" becomes "pintor"
                    translated = translated.replace(/\/a\b/g, '');
                  }
                  // else: keep as-is (with /a) if sex is unknown
                  return translated;
                })
                .join(', ');
